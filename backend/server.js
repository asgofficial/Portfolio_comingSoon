...
// Bumps and returns the site-wide visit counter that powers the
// "LOG ENTRY — 0XX" readout on the coming-soon page.
app.post('/api/visit', (req, res) => {
  try {
    const { count } = incrementVisitCount.get()
    res.json({ ok: true, visitCount: count })
  } catch (err) {
    console.error('visit increment failed:', err.message)
    const fallback = getVisitCount.get()?.count ?? 1
    res.status(500).json({ ok: false, error: 'Could not record visit.', visitCount: fallback })
  }
})
